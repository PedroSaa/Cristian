...
/// <summary>
/// Protects the TOTP MFA secret at rest using AES-GCM (authenticated encryption).
/// The 256-bit key is derived (SHA-256) from <c>Security:MfaEncryptionKey</c> in configuration.
/// Stored format: base64( nonce[12] | tag[16] | ciphertext ). A random nonce per call means
/// the same secret never yields the same ciphertext.
/// If no key is configured the service still constructs (so the app boots with MFA off),
/// but Protect/Unprotect throw — activating MFA without a key is a configuration error.
/// </summary>
public sealed class MfaSecretProtector : IMfaSecretProtector
{
    ...
    {
        var configured = configuration["Security:MfaEncryptionKey"];
    ...
    public string Unprotect(string ciphertext)
    {
        ...
        if (combined.Length < NonceSize + TagSize)
            throw new CryptographicException("El secreto MFA cifrado tiene un formato inválido.");
    ...
    private byte[] RequireKey()
        => _key ?? throw new InvalidOperationException(
            "Security:MfaEncryptionKey no está configurada; no se puede cifrar ni descifrar el secreto MFA.");
}
